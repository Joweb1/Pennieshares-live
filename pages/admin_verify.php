...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'verify') {
        ...
        if ($proofId && $userId) {
            try {
                // Update user status
                $stmt = $pdo_mysql->prepare("UPDATE users SET status = 2 WHERE id = :user_id");
                $stmt->execute([':user_id' => $userId]);

                // Update payment proof status
                $stmt = $pdo_mysql->prepare("UPDATE payment_proofs SET status = 2 WHERE id = :proof_id");
